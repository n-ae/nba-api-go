...
This isn't a claim that generated code is now defect-free everywhere - it's specifically these three, previously-documented, previously-confirmed families. `tools/generator/fieldtypes.json`/`fieldtype_overrides.json` cover 854 fields with varying levels of verification confidence (see `CHANGELOG.md`'s `[2.0.0]` notes); `videoevents.go` still has an unrelated, open field-*naming* issue (not a type defect). See the [maintainability assessment](./docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT_2026-07-19_2363f46.md) for the analysis that started this fix, and [CHANGELOG.md](./CHANGELOG.md) for the complete `[2.0.0]` change list.
