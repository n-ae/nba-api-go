...
> **What "100% coverage" means here**: all 141 endpoints are implemented, type-safe, and covered by generated parsing tests against fixtures synthesized from each endpoint's own metadata - it does **not** currently mean all 141 are reachable against live `stats.nba.com` traffic. A full reachability sweep on 2026-07-22 (raw requests, no SDK, one per endpoint) found only **5 of 141 endpoints respond at all**, from every network tested so far (a residential/business ISP and GitHub Actions runners alike) - the other 136 hang to a hard timeout. This looks like upstream bot-defense on NBA.com's side, not a bug in this library, but it means most endpoints won't return real data against the live API right now, regardless of client. See [`tests/integration/README.md`](./tests/integration/README.md#full-141-endpoint-reachability-sweep-2026-07-22) for the full methodology and results.
...
```go
import "github.com/n-ae/nba-api-go/v3/pkg/stats/parameters"
