...
**Fixed on `main` (unreleased, targeting v2.0.0) for most affected endpoints** by regenerating from a corrected field-type dictionary (`tools/generator/fieldtypes.json` / `fieldtype_overrides.json`) - see `CHANGELOG.md`'s `[Unreleased]` section. This is a source-breaking change (public struct field types), so it isn't in any tagged release yet; **if you're on a tagged release (v1.3.0 or earlier), all of the below still applies.**

The generator infers a Go type for each field from its NBA.com column name (e.g. names containing common stat abbreviations are typed `float64`), and that heuristic got some fields wrong. On tagged releases, this reads the value silently instead of erroring:

- **Display-name fields typed `float64`.** `DISPLAY_FIRST_LAST`, `DISPLAY_LAST_COMMA_FIRST`, `DISPLAY_FI_LAST`, `PLAYER_NAME_LAST_FIRST`, and similar fields contain a name string (e.g. `"Nikola Jokić"`); the field is declared `float64`, and parsing a name string produces `0`, not an error. **Fixed on `main`** in `PlayerDashPtShots` (`PLAYER_NAME_LAST_FIRST`) and `CommonPlayerInfoV2` (`DISPLAY_FIRST_LAST`/`DISPLAY_LAST_COMMA_FIRST`/`DISPLAY_FI_LAST`). **Still defective** (on `main` and every tagged release) in `CommonAllPlayers`/`CommonAllPlayersV2`/`CommonPlayerInfo` - regenerating those three is blocked on unrelated drift, tracked separately.
- **Textual range-bucket fields typed `int`.** `SHOT_CLOCK_RANGE`, `DRIBBLE_RANGE`, `CLOSE_DEF_DIST_RANGE`, `TOUCH_TIME_RANGE`, `SHOT_DIST_RANGE`, and `SHOT_ZONE_RANGE` hold text buckets (e.g. `"24-22"`, `"Very Tight"`); the field is declared `int`, and parsing text produces `0`. **Fixed on `main`** in `PlayerDashPtShots`, `TeamDashPtShots`, `PlayerVsPlayer`, and `ShotChartLineupDetail`. **Still defective** in `ShotChartDetail` (`SHOT_ZONE_RANGE`) - same blocked-on-unrelated-drift situation.
- **Decimal percentage/rating fields typed `string`.** A number of fields matching rating/frequency/percentage naming patterns (e.g. `FG_PCT_RA`, `FG_PCT_IN_PAINT`) are declared `string` and formatted with `%.0f`, so a value like `0.357` is stored as `"0"` - the decimal precision is lost, not merely stringified. **Fixed on `main`** in `LeagueDashPlayerShotLocations` and `LeagueDashTeamShotLocations` (the only two endpoints with these fields).

These are public struct field types in already-tagged releases, so correcting them is a source-breaking change and is deferred to a planned major version rather than silently patched. Until v2.0.0 ships: **if you're on a tagged release, treat any field in these three families as unreliable, and verify field-by-field against a live response for any endpoint your application depends on.** See the [maintainability assessment](./docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT_2026-07-19_2363f46.md) for the full analysis and fix plan, and [CHANGELOG.md](./CHANGELOG.md) for what has and hasn't shipped.
