...
### Changed
- Closes the "smaller, separate follow-up" `[2.0.0]` left open: the other 3 of 4 hand-written endpoints without generator metadata - `commonplayerinfo.go`, `playergamelog.go`, `teamgamelog.go` - now call `findResultSet`/`validateHeaders` against each result set's own `jsonTags`, matching the pattern generated code already uses (`playercareerstats.go` and, as of the fix above, `leagueleaders.go` are handled separately - see `Fixed`). All 6 hand-written endpoints now validate headers in some form.
- **Live verification of the header-validation risk called out in `[2.0.0]` is partially discharged, not fully closed.** `leagueleaders.go` (above) was exercised end-to-end against live `stats.nba.com` traffic and is now confirmed correct for both `PerMode` variants tested. `commonplayerinfo.go`/`playergamelog.go`/`teamgamelog.go` got the same mechanical header-validation code as the generated endpoints but could not be exercised against live traffic in the same session - `stats.nba.com` began rate-limiting/dropping connections from the testing environment after a handful of requests (curl requests to the same endpoints later timed out with zero bytes received). The generated-endpoint surface (121+ files) remains entirely unverified live, same as `[2.0.0]` originally noted. Given what `leagueleaders.go` turned up (an envelope-shape bug and a parameter-dependent column set that plain header validation doesn't handle), treat this as evidence the remaining live-verification backlog is worth prioritizing, not a formality.
