...
### Known limitation found while cross-referencing (not yet fixed)
- The flat `FIELD_NAME -> Go type` dictionary cannot represent fields whose correct type legitimately differs by endpoint. `OREB`/`DREB`/`REB`/`AST`/`STL`/`BLK`/`PF`/`PTS` are committed as `float64` in ~90 endpoints (per-game averages) but `int` in exactly 8 box-score endpoints (single-game counts) - both are correct in their own context; this is not a bug to fix by picking one. A future per-endpoint (or per-result-set) override layer on top of `fieldtypes.json` is needed before these 8 endpoints can be safely regenerated; they're intentionally excluded from the current dictionary's corrections.
