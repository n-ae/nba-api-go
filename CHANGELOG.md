...
- **Not yet regenerated** (7 files, still on the old, partly-wrong types): `videoevents.go` (see above), `shotchartdetail.go`/`commonallplayers.go`/`commonallplayersv2.go`/`teaminfocommon.go` (each has at least one more field-type disagreement with committed code beyond what's been verified so far - `shotchartdetail.go`'s `GAME_EVENT_ID`/`LOC_X`/`LOC_Y`/etc., `commonallplayers[v2].go`'s `PERSON_ID`/`ROSTERSTATUS`/etc., `teaminfocommon.go`'s `W`/`L`/rank fields), `gamerotation.go` (a doc-comment-only diff), `leaguedashplayerstats.go` (a parameter-handling logic diff unrelated to types). None of these affect the 128 files regenerated so far.
...
- Endpoints not listed above (including the 7 "not yet regenerated" files, and the 6 hand-written endpoints with no generator metadata: `CommonPlayerInfo`, `PlayerCareerStats`, `PlayerGameLog`, `TeamGameLog`, `LeagueLeaders`, `InternationalBroadcasterSchedule`) are unaffected by this change.
