...
- **Not yet regenerated** (6 files, still on the old, partly-wrong types): `videoevents.go` (see above), `commonallplayers.go`/`commonallplayersv2.go`/`teaminfocommon.go` (each has at least one more field-type disagreement with committed code beyond what's been verified so far - `commonallplayers[v2].go`'s `PERSON_ID`/`ROSTERSTATUS`/etc., `teaminfocommon.go`'s `W`/`L`/rank fields), `gamerotation.go` (a doc-comment-only diff), `leaguedashplayerstats.go` (a parameter-handling logic diff unrelated to types). None of these affect the 129 files regenerated so far.
...
- Endpoints not listed above (including the 6 "not yet regenerated" files, and the 6 hand-written endpoints with no generator metadata: `CommonPlayerInfo`, `PlayerCareerStats`, `PlayerGameLog`, `TeamGameLog`, `LeagueLeaders`, `InternationalBroadcasterSchedule`) are unaffected by this change.
