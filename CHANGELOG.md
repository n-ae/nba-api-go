...
**Patch, CI only - no runtime or test source changes**: scopes the release install-smoke workflow's empty-input manual dispatch fallback to v3 tags, so it actually matches the "nearest reachable v3 tag" behavior its own description promises - verified in a throwaway clone against a simulated future major-version tag. No caller-visible behavior changed in this release. Closes the one finding from the `2026-07-23` (`4ab1ce1`) maintainability assessment.
...
- **`release-install-smoke.yml`'s manual `tag` input was interpolated directly into the step's shell source via `${{ github.event.inputs.tag }}`**, the script-injection-prone pattern GitHub's own Actions security-hardening guidance recommends against for semi-trusted context values. Now passed through `env: INPUT_TAG` and read as an ordinary shell variable instead. Found by the `2026-07-23` (`fc58431`) maintainability assessment (finding #21).
- **`release-install-smoke.yml`'s `go mod tidy` step was bounded (`timeout-minutes: 3`) but not retried**, despite being able to hit the same `sum.golang.org`/module-proxy checksum lookups that motivated the `go get` step's retry logic - a transient propagation-delay failure there would fail the job outright rather than self-heal. Now wrapped in the same 5-attempt/15-30-45-60s-backoff/`timeout 60s`-per-attempt pattern as `go get`; `go build`/the smoke binary remain single-attempt, since a compile or runtime failure there is deterministic and shouldn't be retried. Found by the `2026-07-23` (`fc58431`) maintainability assessment, a residual gap the prior (`7d6702b`) cycle's own review had already predicted.

### Changed
- **`release-install-smoke.yml`'s `actions/setup-go` step now sets `cache: false`.** The job's stated purpose is proving the tagged module is fetchable/buildable exactly as a first-time external consumer would see it; `setup-go`'s default (`cache: true`) restores a GitHub-managed Go module/build cache, which doesn't currently mask anything (a just-published tag can't already be cache-populated) but weakened that framing's accuracy. Disabled so the verification claim matches what's actually being tested.
...
- **`.github/workflows/fuzz.yml`'s `upload-artifact` step upgraded from `v4` to `v7.0.1`**, resolving the Node.js 20→24 deprecation-forcing warning observed on real runs this cycle, and gains explicit `if-no-files-found: warn` (previously implicit) so a genuine fuzz-step failure that somehow produces no corpus file is itself surfaced rather than silently skipped. `v5`-`v7`'s breaking changes are Node.js runtime bumps only - no change to the `name`/`path`/`retention-days` inputs already in use, confirmed via the action's own release notes before upgrading.
