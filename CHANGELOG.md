...
- Endpoints not listed above (including `videoevents.go`, and the 6 hand-written endpoints with no generator metadata: `CommonPlayerInfo`, `PlayerCareerStats`, `PlayerGameLog`, `TeamGameLog`, `LeagueLeaders`, `InternationalBroadcasterSchedule`) are unaffected by this change.
