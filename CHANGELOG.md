...
- **Not yet regenerated** (14 files, still on the old, partly-wrong types): `playerdashboardbygeneralsplits.go`/`teamdashboardbygeneralsplits.go`/`boxscoresummaryv2.go` (currently `interface{}`-typed placeholders with zero parsing logic - regenerating these is a pure improvement, not just a type fix, and is being handled separately so it isn't buried in this batch), `playbyplayv3.go`/`scoreboardv3.go`/`videoevents.go` (camelCase JSON field naming mismatches vs. committed PascalCase), `leaguestandings.go`/`leaguestandingsv3.go` (an unrelated exported-vs-unexported field name mismatch, `StrLongHomeStreak` vs `strLongHomeStreak`), `shotchartdetail.go`/`commonallplayers.go`/`commonallplayersv2.go`/`teaminfocommon.go` (each has at least one more field-type disagreement with committed code beyond what's been verified so far - `shotchartdetail.go`'s `GAME_EVENT_ID`/`LOC_X`/`LOC_Y`/etc., `commonallplayers[v2].go`'s `PERSON_ID`/`ROSTERSTATUS`/etc., `teaminfocommon.go`'s `W`/`L`/rank fields), `gamerotation.go` (a doc-comment-only diff), `leaguedashplayerstats.go` (a parameter-handling logic diff unrelated to types). None of these affect the 121 files regenerated in this change.

### Migration guide (draft, for the eventual v2.0.0 release)
...
- Endpoints not listed above (including the 14 "not yet regenerated" files, and the 6 hand-written endpoints with no generator metadata: `CommonPlayerInfo`, `PlayerCareerStats`, `PlayerGameLog`, `TeamGameLog`, `LeagueLeaders`, `InternationalBroadcasterSchedule`) are unaffected by this change.
