...
**Current Status**: `main` is a few commits ahead of the latest tagged release, `v3.1.13` (`fc58431`) - unreleased, CI-only fixes (all in `.github/workflows/release-install-smoke.yml`) for two new findings the `2026-07-23` (`fc58431`) maintainability assessment found in `v3.1.13`'s own manual-dispatch hardening, plus a residual gap that same assessment cycle's own review had predicted. Finding #20: `v3.1.13` closed the prior cycle's finding #19 (validate the manual-dispatch `tag` input) with a bare `case "$tag" in v*) ;; ...` prefix check - looser than this same lineage's own prior-cycle recommendation (`v[0-9]*)`) - so a hypothetical branch merely named e.g. `vfeature` would still have passed it and could resolve via `go get`'s pseudo-version handling; fixed by requiring a real semantic-version shape plus a `git show-ref --verify` match against an actual `refs/tags/` ref, with the resolved commit now echoed into the job log. Finding #21: that same step interpolated the manual `tag` input directly into its generated shell script via `${{ github.event.inputs.tag }}` - the exact script-injection-prone pattern GitHub's own Actions security-hardening guidance warns against, missed across four prior CI-hardening cycles; fixed by reading it through `env: INPUT_TAG` instead. Also fixed in the same pass: `go mod tidy` (which can hit the same `sum.golang.org` propagation delay `go get`'s retry logic exists for) now retries with the same backoff instead of only being time-bounded, and `actions/setup-go`'s default caching is disabled so the job's "clean first consumer" framing matches what it actually verifies. This is the first cycle where a finding traces back to this same lineage's own prior-cycle recommendation being implemented more loosely than written, rather than a fresh miss - see `docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT.md` for the full account (grade **A-, held - third consecutive cycle**). See `CHANGELOG.md`'s `[Unreleased]` section for the fixes themselves.
...
**Latest tagged release**: v3.1.13 (`fc58431`) - `main` is a few commits ahead via unreleased CI-only fixes (in `release-install-smoke.yml`) for two new findings (#20, #21) the `2026-07-23` (`fc58431`) maintainability assessment found in `v3.1.13`'s own hardening pass, plus a residual `go mod tidy` retry gap that same assessment cycle's own review had predicted; see "Current Status" near the top of this file for the full account. `v3.1.13` itself was a patch, CI only, closing findings #16-#19 from the `2026-07-23` (`7d6702b`) assessment; see `CHANGELOG.md`'s `[3.1.13]` section. `v3.1.12` through `v3.1.10` were patches too, closing manual-dispatch tag-resolution, `sum.golang.org`-propagation-delay-retry, and repo-wide CI supply-chain-hardening findings across three assessment cycles; see `CHANGELOG.md`'s `[3.1.10]` through `[3.1.12]` sections. `v3.1.9` through `v3.1.3` were all patches too - no Go `pkg/` API changes in any of them - covering a `BaseURL` secret-disclosure defect closed over four cycles, one further secret-echo instance, and the addition of scheduled fuzz CI; see `CHANGELOG.md`'s `[3.1.3]` through `[3.1.9]` sections, or "Current Status" for the summarized chain. `v3.1.2` was a patch closing both new findings from the `2026-07-22` (`9eb3a9a`) maintainability assessment: `client.NewClient`'s `BaseURL` validation now requires an absolute `http`/`https` URL with a host, and `tools/generator` gained a test (`TestEndpointPathMatchesNameConvention`) that catches a metadata-level endpoint-path typo before it ships; see `CHANGELOG.md`'s `[3.1.2]` section for the full list. `v3.1.1` was a patch too (docs, `tools/generator`'s own internal test coverage, `DefaultUserAgent` value bump); see `CHANGELOG.md`'s `[3.1.1]` section. `v3.1.0` shipped no Go `pkg/` API changes either (two `**Breaking (HTTP API)**` behavior changes in `cmd/nba-api-server` plus a URL-path bugfix and a generated-test-coverage jump; see `CHANGELOG.md`'s `[3.1.0]` section). The last breaking major release was `v3.0.0` (`NewClient` signature change, `/v3` module path); see `CHANGELOG.md`'s `[3.0.0]` section for that migration guide.
**Go Version**: 1.26.5+ (the `go` directive in `go.mod`; older toolchains cannot build this module)
**Stability**: See the Versioning/API Stability section above - v3.1.13 through v3.1.3, and v3.1.2/v3.1.1/v3.1.0, shipped no Go `pkg/` API breaking changes (each confirmed green by `apidiff.yml`); v3.0.0 was a deliberate major-version break (`NewClient` signature, `/v3` module path); v2.2.0/v2.1.0 shipped no breaking changes; v2.0.0 was a deliberate major-version break (public struct field types); v1.2.0 also broke source compatibility but in a minor release, documented with a retroactive compatibility note in `CHANGELOG.md`; v1.3.0 shipped no breaking changes

See `CHANGELOG.md` for full version history.

---

**This file last updated**: 2026-07-23 (refreshing "Current Status"/"API Stability"/"Version Information" through `v3.1.13` plus its own unreleased follow-up fixes (findings #20/#21, plus a residual `go mod tidy` retry gap) - this file had gone stale at the `v3.1.9` era for four releases; found and fixed as a direct follow-up to applying this cycle's assessment quick-wins, not itself a finding from the assessment document)
**Maintainability grade**: tracked in the current assessment (see the header of this file), not duplicated here
**Next assessment**: the current assessment of record (`docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT.md`, a stable path that never changes - see that file's naming-convention note) as of this update covers revision `fc58431` (tag `v3.1.13`) - grade **A-, held - third consecutive cycle** since the `v3.1.10` recovery. Its two new findings (#20, the manual-dispatch tag guard only prefix-matching rather than proving an actual tag; #21, that same input interpolated directly into generated shell rather than mediated through `env:`) plus a residual `go mod tidy` retry gap its own review predicted are already closed on `main` (not yet in a tagged release) - the assessment document itself was deliberately not updated to reflect that closure (out of scope for the review pass that found it; the next assessment cycle will pick it up as already-closed). Next assessment cycle due quarterly unless another release ships first
