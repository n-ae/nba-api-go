...
**Current Status**: `main` is ahead of the latest tagged release (`v1.3.0`) with v2.0.0 work in progress: explicit generator type metadata and per-endpoint overrides have landed, and 124 of 135 metadata-covered SDK endpoints have been regenerated with corrected field types (a breaking change to public struct fields, not yet tagged). 11 endpoints remain on old types pending further work, plus 9 `interface{}` fields scattered across the 6 hand-written endpoints with no generator metadata (see `CHANGELOG.md`'s `[Unreleased]` section for the full list and reasons); result-set-name keying and the other v2.0.0 items are not yet done.
