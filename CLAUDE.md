...
**Current Status**: `main` is one commit ahead of the latest tagged release, `v3.1.9` (`e3ee47c`) - an unreleased, CI-only fix (PR #75, `768cb79`) for a bug the `2026-07-23` (`e3ee47c`) maintainability assessment found in `v3.1.9` itself: `.github/workflows/fuzz.yml`'s corpus-upload step condition (`if: steps.fuzz.outcome == 'failure'`) omitted the `failure()` status-check function GitHub Actions requires to override its implicit `success()` default, so the upload could never actually run on a genuine fuzz failure - the one case the whole mechanism exists for. Fixed to `if: failure() && steps.fuzz.outcome == 'failure'` and verified live (not just YAML-validated): a throwaway branch with the fuzz step swapped for one that writes a sentinel corpus file and exits 1 confirmed, via a real `workflow_dispatch` run, that the upload step now runs and the resulting artifact contains exactly that sentinel file; a second real run on `main` after merging confirmed the success path still correctly skips the upload. This is the first defect this assessment lineage has found in a shipped release since the `v3.1.6`→`v3.1.7` BaseURL fix-that-verified-the-wrong-thing, and it traces directly back to this same lineage's own prior-cycle recommendation (which specified the outcome check but not the required `failure()` override) - it dropped the assessment grade to **B+, from A-** (see `docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT.md` for the full account, including reconciliation against an unsolicited external review that first flagged the bug; `docs/archive/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT_2026-07-22_0e35c33.md` for the outgoing `v3.1.8`-era A- assessment this one supersedes). See `CHANGELOG.md`'s `[Unreleased]` section for the fix itself.
...
**Latest tagged release**: v3.1.9 (`e3ee47c`) - `main` is one commit ahead via an unreleased CI-only fix (PR #75, `768cb79`) for a bug the `2026-07-23` maintainability assessment found in `v3.1.9`'s own fuzz-workflow hardening; see "Current Status" near the top of this file for the full account. `v3.1.9` itself was a patch, CI only, closing both findings from the `2026-07-22` (`0e35c33`) assessment; see `CHANGELOG.md`'s `[3.1.9]` section. `v3.1.8` through `v3.1.3` were all patches too - no Go `pkg/` API changes in any of them - covering a `BaseURL` secret-disclosure defect closed over four cycles, one further secret-echo instance, and the addition of scheduled fuzz CI; see `CHANGELOG.md`'s `[3.1.3]` through `[3.1.8]` sections, or "Current Status" for the summarized chain. `v3.1.2` was a patch closing both new findings from the `2026-07-22` (`9eb3a9a`) maintainability assessment: `client.NewClient`'s `BaseURL` validation now requires an absolute `http`/`https` URL with a host, and `tools/generator` gained a test (`TestEndpointPathMatchesNameConvention`) that catches a metadata-level endpoint-path typo before it ships; see `CHANGELOG.md`'s `[3.1.2]` section for the full list. `v3.1.1` was a patch too (docs, `tools/generator`'s own internal test coverage, `DefaultUserAgent` value bump); see `CHANGELOG.md`'s `[3.1.1]` section. `v3.1.0` shipped no Go `pkg/` API changes either (two `**Breaking (HTTP API)**` behavior changes in `cmd/nba-api-server` plus a URL-path bugfix and a generated-test-coverage jump; see `CHANGELOG.md`'s `[3.1.0]` section). The last breaking major release was `v3.0.0` (`NewClient` signature change, `/v3` module path); see `CHANGELOG.md`'s `[3.0.0]` section for that migration guide.
**Go Version**: 1.26.5+ (the `go` directive in `go.mod`; older toolchains cannot build this module)
**Stability**: See the Versioning/API Stability section above - v3.1.9 through v3.1.3, and v3.1.2/v3.1.1/v3.1.0, shipped no Go `pkg/` API breaking changes (each confirmed green by `apidiff.yml`); v3.0.0 was a deliberate major-version break (`NewClient` signature, `/v3` module path); v2.2.0/v2.1.0 shipped no breaking changes; v2.0.0 was a deliberate major-version break (public struct field types); v1.2.0 also broke source compatibility but in a minor release, documented with a retroactive compatibility note in `CHANGELOG.md`; v1.3.0 shipped no breaking changes

See `CHANGELOG.md` for full version history.

---

**This file last updated**: 2026-07-23 (refreshing "Current Status"/"API Stability"/"Version Information" through `v3.1.9` plus the unreleased fuzz-workflow fix, `768cb79` - this file had gone stale at the `v3.1.2` era since PR #55, seven releases behind; found and fixed as a direct follow-up to closing this cycle's assessment loop, not itself a finding from the assessment document)
**Maintainability grade**: tracked in the current assessment (see the header of this file), not duplicated here
**Next assessment**: the current assessment of record (`docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT.md`, a stable path that never changes - see that file's naming-convention note) as of this update covers revision `e3ee47c` (tag `v3.1.9`) - grade **B+, down from A-**, the first grade movement in this lineage's history and the first defect found in a shipped release since the `v3.1.6`→`v3.1.7` cycle. Its one live finding (#14, `fuzz.yml`'s corpus-upload condition missing `failure()`) is already closed on `main` (PR #75, `768cb79`) and verified live via a deliberately-failing `workflow_dispatch` run, though not yet in a tagged release - the assessment document itself was deliberately not updated to reflect that closure (out of scope for the review pass that found it; the next assessment cycle will pick it up as already-closed). Next assessment cycle due quarterly unless another release ships first
