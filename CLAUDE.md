...
**Latest tagged release**: v3.1.0 - no Go `pkg/` API changes; two `**Breaking (HTTP API)**` behavior changes in `cmd/nba-api-server` plus a URL-path bugfix and a generated-test-coverage jump; see `CHANGELOG.md`'s `[3.1.0]` section for the full list. The last breaking major release was `v3.0.0` (`NewClient` signature change, `/v3` module path); see `CHANGELOG.md`'s `[3.0.0]` section for that migration guide.
**Go Version**: 1.26.5+ (the `go` directive in `go.mod`; older toolchains cannot build this module)
**Stability**: See the Versioning/API Stability section above - v3.1.0 shipped no Go `pkg/` API breaking changes; v3.0.0 was a deliberate major-version break (`NewClient` signature, `/v3` module path); v2.2.0/v2.1.0 shipped no breaking changes; v2.0.0 was a deliberate major-version break (public struct field types); v1.2.0 also broke source compatibility but in a minor release, documented with a retroactive compatibility note in `CHANGELOG.md`; v1.3.0 shipped no breaking changes

See `CHANGELOG.md` for full version history.

---

**This file last updated**: 2026-07-22 (`v3.1.0` release: generated server handlers, generated endpoint test coverage, URL-path fix)
**Maintainability grade**: tracked in the current assessment (see the header of this file), not duplicated here
**Next assessment**: the current assessment of record (`docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT_2026-07-22_180a3db.md`, grade A-) reviews `main` as of `180a3db`, tag `v3.1.0` - next assessment cycle due quarterly unless another release ships first; see that document's §5 "Recommended order of work" for what it flagged as still open (`README.md`'s self-representation gap, a few stale doc links/references, `tools/generator/README.md` not reflecting handler/test generation)
