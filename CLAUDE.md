...
**Current Status**: `main` is at the latest tagged release, `v2.2.0` (fixes `client.Config.Timeout` being silently ignored with a custom `HTTPClient`; see `CHANGELOG.md`'s `[2.2.0]` section) - itself a reliability follow-up to `v2.1.x`/`v2.0.0`. **`main` currently has unreleased work past that tag**: `1592e7e` narrowed the scheduled live-drift CI workflow to the two endpoints reachable from GitHub Actions runners, and `CHANGELOG.md`'s `[Unreleased]` section now also contains a **breaking API change** (`client.NewClient`/`stats.NewClient`/`live.NewClient` return `(*Client, error)` instead of `*Client`, so an invalid `BaseURL` fails at construction instead of on first request) that has deliberately **not** been tagged as a release - it needs a `v3.0.0` decision (major version bump, `/v3` module path update, migration guide) before it ships; see `[Unreleased]`'s Migration guide for the full rationale and the `v2.0.0`/`v2.1.0` incident it's deliberately avoiding repeating. All 135 metadata-covered SDK endpoints (including `videoevents.go`, regenerated via `tools/generator/fieldname_overrides.json`) have field names/types verified to match generator output. All 6 hand-written endpoints without generator metadata now validate headers in some form (`commonplayerinfo`/`playergamelog`/`teamgamelog`/`playercareerstats` call `validateHeaders` like generated code; `leagueleaders` needed a different, header-name-driven approach - see below). **The header-validation piece introduced in `v2.0.0` is real, deliberate new failure-mode surface, and live verification against real NBA.com responses remains only partially discharged** - see `CHANGELOG.md`'s `[2.0.0]` section for the original risk callout. Live-verifying `leagueleaders.go` against real `stats.nba.com` traffic (part of `v2.1.0`) surfaced and fixed two real bugs (a singular-`resultSet` envelope shape unique to this endpoint, and a `TEAM_ID` column the struct lacked) plus a genuine API quirk (its column set varies by `PerMode`). That's 1 of 142 endpoint files checked against live traffic; the other 5 hand-written endpoints (`commonplayerinfo`, `playergamelog`, `teamgamelog`, `playercareerstats`, `internationalbroadcasterschedule`) and the entire 121+-file generated-endpoint surface remain unverified. **This is not just a rate-limiting inconvenience**: re-attempted 2026-07-22 from a second, unrelated network (a residential/business ISP, not a cloud/CI IP range), and `commonplayerinfo`/`playergamelog`/`playercareerstats`/`teamgamelog` all hung to a hard timeout on the very first request (confirmed via raw `curl`, bypassing the SDK, so it isn't an SDK bug) while `leagueleaders` kept succeeding from the same IP in the same session - see `tests/integration/README.md`'s "Known live-traffic blocks" section. That looks like `stats.nba.com` defending specific endpoint paths more aggressively (Akamai bot detection) rather than blocking by client IP class, and it means live-verifying those 4 endpoints isn't simply a matter of running the suite from a non-CI machine - the same block has now been reproduced from two independent, unrelated networks.
...
**Latest tagged release: v2.2.0** - no breaking changes (a `client.Config.Timeout` reliability fix; see `CHANGELOG.md`'s `[2.2.0]` section). The last major version bump was `v2.0.0`, for real, extensive breaking changes (public struct field types across ~130 generated endpoint files; see `CHANGELOG.md`'s `[2.0.0]` migration guide). Note that v1.2.0 also shipped a source-breaking change, but in a *minor* release (`stats.Config`/`live.Config` field types, `models.NewAPIError`/`HTTPStatusToError` signatures) - see `CHANGELOG.md`'s retroactive compatibility note under `[1.2.0]`. Treat the "strict semver" promise below as the target, not an unconditional guarantee of the historical record. **`main` currently carries an unreleased, deliberately un-tagged breaking change** (`NewClient`'s signature - see `CHANGELOG.md`'s `[Unreleased]` section) pending a `v3.0.0` decision; don't tag any release from `main` without resolving that first.
...
**Latest tagged release**: v2.2.0 (`main` is one commit past this tag; see `CHANGELOG.md`'s `[Unreleased]` section for what's changed since - including an unreleased, deliberately un-tagged breaking change pending a `v3.0.0` decision)
**Go Version**: 1.26.5+ (the `go` directive in `go.mod`; older toolchains cannot build this module)
**Stability**: See the Versioning/API Stability section above - v2.2.0/v2.1.0 shipped no breaking changes; v2.0.0 was a deliberate major-version break (public struct field types); v1.2.0 also broke source compatibility but in a minor release, documented with a retroactive compatibility note in `CHANGELOG.md`; v1.3.0 shipped no breaking changes

See `CHANGELOG.md` for full version history.

---

**This file last updated**: 2026-07-22 (`NewClient` breaking-change prep, unreleased)
**Maintainability grade**: tracked in the current assessment (see the header of this file), not duplicated here
**Next assessment**: the current assessment of record (`docs/MAINTAINABLE_ARCHITECT_V4_ASSESSMENT_2026-07-22_1592e7e.md`, grade B+) already reviews `v2.2.0` (the `Config.Timeout`/custom-`HTTPClient` fix) and the post-tag live-drift workflow narrowing (`1592e7e`); next assessment due quarterly unless another release ships first
