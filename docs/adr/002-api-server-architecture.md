...
>
> **Update (2026-07-22):** "Phase 2: Full Coverage" below - *"Generate
> handlers for all N endpoints"* - is done, ~2 years after this ADR first
> planned it. All 142 HTTP routes (`cmd/nba-api-server/generated_*.go`,
> `generated_dispatch.go`) are now generated by `tools/generator` from the
> same metadata that drives SDK generation, replacing ~4,358 hand-written
> LOC that had accumulated at effectively 0% real test coverage instead -
> the "decide the server's fate" item this repo's maintainability
> assessments carried unresolved for 5+ cycles. See `CHANGELOG.md`'s
> `[Unreleased]` section for what changed, including two real HTTP-API
> behavior fixes that came with standardizing 142 previously
> inconsistent hand-written handlers into one generated pattern.
