> **Superseded.** This assessed revision `7d6702b` (tag `v3.1.12`, grade A-, held - second consecutive
> cycle at A- since the `v3.1.10` recovery). The current assessment of record is
...
> covers revision `fc58431` (tag `v3.1.13`, **grade A-, held - third consecutive cycle**) at the time
> it was written, and will cover whatever the current cycle is by the time you're reading this.
> Retained here for history; see that document's section 2 ("Verification ledger") for the
> item-by-item status of the findings below - findings #16-#19 (the release-smoke retry loop's
> unbounded worst case, the `fuzz.yml` concurrency-comment inaccuracy, the unpinned
> `golangci-lint-action`, and the unvalidated manual `tag` input) were all closed by `v3.1.13`
> (PR #83/#84), each fixed exactly as this file's §5 recommended - except finding #19, whose fix
> (`case "$tag" in v*) ;; ...)` prefix matching) turned out to be weaker than what this file's own
> §5 actually asked for (`v[0-9]*)`), and a third external review (this time of `v3.1.13`) correctly
> caught that gap as a new P1: the prefix check accepts any string starting with `v`, including a
> hypothetical branch name like `vfeature`, so it does not by itself prove the manually-supplied
> input is an existing, immutable release tag. See the current assessment of record for the full
> reconciliation.
