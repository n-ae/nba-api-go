> **Superseded.** This assessed revision `04537f4` (tag `v3.1.10`, grade A-, recovered from B+) - the
> cycle that closed finding #14 (the fuzz-job corpus-upload condition) and downgraded two of an
> external review's P1 findings to lower severity on direct log evidence, while confirming a new,
> latent finding #15 (manual-dispatch tag resolution silently defaulting to the triggering branch).
> The current assessment of record is
...
> covers revision `7d6702b` and later (tag `v3.1.12`, **grade A-, held**) at the time it was written,
...
> findings below - finding #15 (manual-dispatch tag resolution) was closed for real by `v3.1.11` (PR
> #78, with a second latent bug - a shallow checkout lacking tag history - found and fixed in the same
> PR on first real dispatch), the `sum.golang.org` propagation-delay flake documented below as
> "informational, not a defect" was proactively hardened against with retry/backoff in `v3.1.12` (PR
> #80) and that retry logic had its first live test on the `v3.1.12` tag push itself, succeeding on
> attempt 2 exactly as designed. A second external review, supplied for the `v3.1.12` cycle, raised a
> new P1 (the retry loop's "~2.5 minutes worst case" is only the sum of sleeps, with no job- or
> command-level timeout bounding a stall rather than a failure) and several P2s, one of which (the
> `fuzz.yml` concurrency-group rationale comment added in `v3.1.12`'s own PR #81 describes a
> same-filesystem race that GitHub's documented one-fresh-VM-per-job model makes structurally
> impossible) was independently confirmed against GitHub's own documentation. See that document's
> section 0 for the full reconciliation.
