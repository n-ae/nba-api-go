> **Superseded.** This assessed revision `1b428f6` (grade A-). The current assessment of record is
...
> document's naming-convention note near the top): it covered revision `b3c605d` and later (tag
> `v3.1.3`, grade A-, unchanged) at the time it was written, and will cover whatever the current cycle
> is by the time you're reading this. Retained here for history; see that document's section 2
> ("Verification ledger") for the item-by-item status of all three findings below - all three closed in
> `v3.1.3`, and section 0 of that document independently verifies an unsolicited external review
> supplied for that cycle (its central finding, a `BaseURL`-error secret-echo defect, turned out to be
> real and broader in scope than the review itself claimed).
