> **Superseded.** This assessed revision `0e400d1` (grade A-). The current assessment of record is
...
> document's naming-convention note near the top): it covered revision `f4801ef` and later (tag
> `v3.1.5`, **grade B+, down from A-** - the first grade change in this lineage's history) at the time
> it was written, and will cover whatever the current cycle is by the time you're reading this.
> Retained here for history; see that document's section 2 ("Verification ledger") for the item-by-item
> status of the two findings below - both genuinely closed by `v3.1.5`, but a third instance of the
> same underlying defect class (`BaseURL` secrets leaking through a `NewClient` error) was found in the
> same cycle: the "safe" unwrapped inner parser reason this file's own fix relied on is not actually
> guaranteed input-free. See that document's section 0 and section 7 for the full account of why the
> grade changed.
...
2. **Both severities are still low-likelihood in practice** for the same reason named every cycle since `b3c605d`: this project's own documented use case (NBA Stats, no auth) gives no caller a reason to put a secret in `BaseURL` today. Real, evidenced, and cheap to fix - not a sign of runtime harm having occurred.
...
- Everything `9eb3a9a`/`180a3db`/`1b428f6`/`b3c605d` already marked not-urgent (live-verifying the 136 unreachable endpoints, HTTP-server independent versioning policy, ecosystem-maturity commentary) remains not-urgent for the same reasons already given in those assessments - not re-litigated here.
