...
// handleLeagueLeadersV2 is generated from tools/generator/metadata; see
// tools/generator/templates/handler.tmpl. Do not hand-edit - regenerate via
// `cd tools/generator && go run . -endpoint LeagueLeadersV2` (or -all-handlers to
...
func (h *StatsHandler) handleLeagueLeadersV2(w http.ResponseWriter, r *http.Request) {
	...
	if raw := r.URL.Query().Get("Scope"); raw != "" {
		vScope = stringPtr(raw)
	}
	var vStatCategory *string
	if raw := r.URL.Query().Get("StatCategory"); raw != "" {
		vStatCategory = stringPtr(raw)
	}
