...
// countHandlerRouteCases parses this package's generated_dispatch.go and
// counts the entries in the generatedDispatch map literal (generated by
// tools/generator - see templates/dispatch.tmpl; the map replaced what
// used to be a hand-maintained switch statement in
// StatsHandler.ServeHTTP). The file has exactly one map[string]func(...)
// composite literal; if that ever changes, this will over- or
// under-count and this test's failure message will point here.
func countHandlerRouteCases(t *testing.T) int {
	t.Helper()

	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, "generated_dispatch.go", nil, 0)
	if err != nil {
		t.Fatalf("failed to parse generated_dispatch.go: %v", err)
	}
